...
In this example we will build a chat executor where the LLM can optionally decide to return the result of a tool call as 
the final answer. This is useful in cases where you have tools that can sometimes generate responses that are acceptable 
as final answers, and you want to use the LLM to determine when that is the case

This examples builds off the base chat executor. It is highly recommended you learn about that executor before going 
through this notebook. You can find documentation for that example here(https://github.com/langchain-ai/langgraph/blob/66b728e83a0661d1ca8f47fad97ef53d3a5f717d/examples/chat_agent_executor_with_function_calling/base.ipynb).

Any modifications of that example are called below with MODIFICATION, so if you are looking for the differences you 
can just search for that.

Note

In this how-to, we will create our agent from scratch to be transparent (but verbose). You can accomplish similar 
functionality using the create_react_agent(model, tools=tool, interrupt_before=["agent" | "tools"], interrupt_after=["agent" | "tools"], 
checkpointer=checkpointer) constructor (https://langchain-ai.github.io/langgraph/reference/prebuilt/#create_react_agent). 
This may be more appropriate if you are used to LangChain’s AgentExecutor(https://python.langchain.com/v0.2/docs/how_to/agent_executor/#concepts) class.
...
"""
We can now wrap these tools in a simple ToolExecutor. This is a real simple class that takes in a ToolInvocation 
and calls that tool, returning the output. A ToolInvocation is any class with tool and tool_input attribute.
"""
...
### MODIFICATION
We add a separate node for any tool call where return_direct=True. The reason this is needed is that after this node 
we want to end, while after other tool calls we want to go back to the LLM.
"""
...
"""
### Compare with above inputs
"""
inputs = {
    "messages": [
        HumanMessage(
            content="what is the weather in Bandung? return this result directly by setting return_direct = True"
        )
    ]
}
